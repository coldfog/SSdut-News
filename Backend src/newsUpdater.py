...
class BaseHandler(tornado.web.RequestHandler):
    ...
    DATABASE_FILE =  'ssdutNews.db' # os.environ['LOCALAPPDATA']+'\\ssdutNews.db'
    __database = None
...
class UpdateHandler(BaseHandler):
    ...
    def update(self, page, ongoing=False, callback=None):
        ...
        def _on_download(response):
            ...
            print "DEBUG", "_on_download", response.code
            if response.code == 200:
                raw_result, entries_num, total_page = self._decode(response.body)
                print response.request.url
                curr_page= int(re.search(r'/p/(\d+)', response.request.url).group(1))
                result = self._encode(raw_result, entries_num, curr_page)

                max_no = max([x["no"] for x in result])
                min_no = min([x["no"] for x in result])

                entries = self.query_news(oldest=(min_no-1), latest=(max_no+1))
                for ret in result:
                    for entry in entries:
                        if ret["no"] == entry["no"]:
                            break;
                    else:
                        print "DEBUG", "no break"
                        self.store_news(**ret)
        ...
        http.fetch(SSDUT_SITE+"/index.php/News/student/p/%d"%page, _on_download)
        ...
        while(gap != 2):
            ...
            if gap % 2 == 0:
                ...
                if nums[end] + gap/2 != nums[end-gap/2]:
                    ...
                    gap = end-start+1
                    continue
            print "DEBUG", "Search hole, return last", min(nums)-1
            return min(nums)-1

        if nums[start]-1 != nums[end]:
            print "DEBUG", "Search hole, return hole", nums[start]-1
            return nums[start]-1
    ...
    def _decode(self, content):
        ...
        '''
        news_pattern = re.compile(r'<tr\ bgcolor=#EEEEEE>[\s\S]+?'+
    ...
    def _get_page_num(self, content):
        ...
        page_info_pattern = re.compile(r'<td width="80%">'+ 
                r'<font color="#666666">共(\d+?) 条记录/(\d+?)页')
        result = page_info_pattern.search(content)
...
class ContentGetHandler(BaseHandler):
    ...
    def _on_download(self, response):
        print "on download"
        if response.code == 200:
            pattern = re.compile(r'<td height="36" align="center" class="title">'+
                    r'(.+?)</td>[\s\S]+?'+
                    r'<td class="content">([\s\S]+)</td>[\s\S]+?'+
                    r'<table width="760" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">')
            result = {}
            result['title'], result['content'] = pattern.search(response.body).group(1, 2)
